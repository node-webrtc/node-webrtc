...
class WavFileReader final : public FakeAudioDevice::Capturer {
 public:
  ...
  bool Capture(rtc::BufferT<int16_t>* buffer) override {
    buffer->SetData(
      FakeAudioDevice::SamplesPerFrame(sampling_frequency_in_hz_),
...
class WavFileWriter final : public FakeAudioDevice::Renderer {
 public:
...
class BoundedWavFileWriter : public FakeAudioDevice::Renderer {
 public:
  BoundedWavFileWriter(std::string filename, int sampling_frequency_in_hz)
  : sampling_frequency_in_hz_(sampling_frequency_in_hz)
  , wav_writer_(filename, sampling_frequency_in_hz, 1)
  , silent_audio_(FakeAudioDevice::SamplesPerFrame(sampling_frequency_in_hz), 0)
...
class DiscardRenderer final : public FakeAudioDevice::Renderer {
 public:
...
FakeAudioDevice::PulsedNoiseCapturer::PulsedNoiseCapturer(int16_t max_amplitude, int sampling_frequency_in_hz)
: sampling_frequency_in_hz_(sampling_frequency_in_hz)
...
bool FakeAudioDevice::PulsedNoiseCapturer::Capture(rtc::BufferT<int16_t>* buffer) {
...
FakeAudioDevice::CreatePulsedNoiseCapturer(int16_t max_amplitude, int sampling_frequency_in_hz) {
  return std::unique_ptr<FakeAudioDevice::PulsedNoiseCapturer>(
    new PulsedNoiseCapturer(max_amplitude, sampling_frequency_in_hz));
}

std::unique_ptr<FakeAudioDevice::Capturer> FakeAudioDevice::CreateWavFileReader(std::string filename, int sampling_frequency_in_hz) {
  return std::unique_ptr<FakeAudioDevice::Capturer>(
    new WavFileReader(filename, sampling_frequency_in_hz));
}

std::unique_ptr<FakeAudioDevice::Capturer> FakeAudioDevice::CreateWavFileReader(std::string filename) {
  int sampling_frequency_in_hz = webrtc::WavReader(filename).sample_rate();
  return std::unique_ptr<FakeAudioDevice::Capturer>(
    new WavFileReader(filename, sampling_frequency_in_hz));
}

std::unique_ptr<FakeAudioDevice::Renderer> FakeAudioDevice::CreateWavFileWriter(std::string filename, int sampling_frequency_in_hz) {
...
FakeAudioDevice::CreateBoundedWavFileWriter(std::string filename, int sampling_frequency_in_hz) {
  return std::unique_ptr<FakeAudioDevice::Renderer>(
...
FakeAudioDevice::FakeAudioDevice(std::unique_ptr<Capturer> capturer, std::unique_ptr<Renderer> renderer, float speed)
