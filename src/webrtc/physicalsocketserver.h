// NOTE(mroberts): https://github.com/js-platform/node-webrtc/issues/362
/*
 ...
 */
#ifndef SRC_PHYSICALSOCKETSERVER_H_
#define SRC_PHYSICALSOCKETSERVER_H_
...
class PhysicalSocketServer : public rtc::SocketServer {
 public:
  ...
  // SocketFactory:
  rtc::Socket* CreateSocket(int type) override;
  ...
  virtual rtc::AsyncSocket* WrapSocket(SOCKET s);
  // SocketServer:
...
class PhysicalSocket : public rtc::AsyncSocket, public sigslot::has_slots<> {
 public:
  ...
  int Bind(const rtc::SocketAddress& bind_addr) override;
  ...
  rtc::AsyncSocket* Accept(rtc::SocketAddress* out_addr) override;
  int Close() override;
  rtc::SocketServer* socketserver() { return ss_; }
 protected:
  int DoConnect(const rtc::SocketAddress& connect_addr);
...
#endif  // SRC_PHYSICALSOCKETSERVER_H_