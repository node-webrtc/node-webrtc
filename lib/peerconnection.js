...
var RTCAudioFrameEvent        = require('./rtcaudioframeevent');
...
function RTCPeerConnection(configuration, constraints) {
  ...
  pc.onaddstream = function(internalMS) {
    this._mediaStreams = this._mediaStreams || {};
    var ms = new RTCMediaStream(internalMS);
    this._mediaStreams[internalMS.id] = ms;
  ...
  pc.onremovestream = function(internalMS) {
    var ms = this._mediaStreams[internalMS.id];
    var removeStreamEvent = new RTCMediaStreamEvent('removestream', {stream: ms});
    self.dispatchEvent(removeStreamEvent);
    this._onremovestream && self._onremovestream(removeStreamEvent);
  };

  pc.onaudioframe = function(id, bits_per_sample, sample_rate, number_of_channels, number_of_frames, buffer) {
      var audioFrameEvent = new RTCAudioFrameEvent('audioframe',
          {id: id, bits_per_sample: bits_per_sample,
              sample_rate: sample_rate, number_of_channels: number_of_channels,
              number_of_frames: number_of_frames, buffer: buffer});
      self.dispatchEvent(audioFrameEvent);
      self._onaudioframe && self._onaudioframe(audioFrameEvent);
  };
  ...
  Object.defineProperties(this, {
    ...
    onremovestream: {
        set: function(onRemoveStream) {
            self._onremovestream = onRemoveStream;
        }
    },
    ...
    onaudioframe: {
        set: function(onAudioFrame) {
            self._onaudioframe = onAudioFrame;
        }
    },
    onencodedvideoframe: {
  ...
  this.onStreamAudioFrame = function(stream, callback) {
      var audioTrackIds = stream.getAudioTracks().map(function(aTracks) {
          return aTracks.id;
      });

      self.addEventListener('audioframe', function(frame) {
          if(audioTrackIds.find(function(tId) {
                  return tId === frame.id
              })) {
              callback(frame);
              return;
      ...
      pc.onStreamAudioFrame(stream._internalMS, callback);
  };
